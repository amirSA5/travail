...
router.post('/Ajout_Attribut', async (req, res) =>{
        try {
            ...
            const attributSousArticle = await AttributTemplate.findOne({sousArticle})
            if(attributNom && attributSousArticle) return res.status(400).json({msg: "This Attribut already exists."})

             
            const newAttribut = new AttributTemplate({Nom,sousArticle})
